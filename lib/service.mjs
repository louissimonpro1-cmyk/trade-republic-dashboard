...
export async function getPerf(range) {
  ...
  let data;
  if (range === "1d") {
    const { sessionDate, points } = buildIntradaySeries(ledger, market);
    data = {
      range, sessionDate,
    ...
    const points = perfSeries(dates, values, ledger.flowsByDate, startIdx);
    data = {
      range, sessionDate: null,
      points: points.map((p) => ({ t: p.t, pct: round3(p.pct), value: round2(p.value) })),
    };
  }
...
export async function getAssetSeries(isin) {
  ...
  const data = { isin, daily, intraday };
